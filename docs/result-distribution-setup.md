...
知己的本地日志、日反馈和复盘仍可正常使用，不依赖飞书或滴答。外部结果分发能力随用户版提供，但**不是解压即用**：每位用户都必须在自己的电脑和账号上完成官方工具安装、授权、目标目录或清单绑定，并显式开启运行配置。主项目或维护者环境中的验收结果，不能替代你在自己的电脑和账号上完成首次设置。

最短启用路径：

1. 按下文分别完成飞书 `lark-cli` 和对应区域滴答/TickTick 官方 MCP 的安装与授权。
2. 在飞书准备“知己”分类目录，在滴答准备唯一的“知己行动”清单，取得非敏感的目录 token 与 `project_id`。
3. 复制配置示例 `.claude/shared/result-distribution-config.example.json` 为 `复盘/.result-distribution-config.json`，填写目标标识；测试完成前保持开关关闭。
4. 分别完成一次脱敏文档和一次脱敏任务测试，确认目标位置、账号可见性和防重正常；完成一次脱敏测试后再打开开关，只启用自己需要的渠道与结果类型。

如果某次内容只想留在电脑上，在本次生成请求中直接说“仅本地”。系统仍完成本地写入和复读，但本轮同时跳过飞书和滴答；这个选择只对本次请求有效，不写入永久标记、配置或状态。

## 给 AI 的执行契约

当用户要求配置或检查结果分发时，AI 应先读取本文件与 `.claude/shared/contracts/result-distribution.md`，并按以下顺序执行：

1. 先验证本地日志、日反馈和复盘路径可写、可复读；外部服务未配置时也不得把本地功能判为不可用。
2. 分别检查飞书和滴答，不把一个渠道的成功或失败推断到另一个渠道。
3. 自动完成安全且可回退的检查、配置示例复制、非敏感目标 ID 获取、脱敏测试和通过后的开关修改。
4. 遇到官方登录、OAuth 同意、企业应用创建、敏感凭证输入、客户端安装或必须由用户目视确认的步骤时暂停。每次只要求用户完成一个动作，并说明官方入口、具体动作、完成标志和禁止发送给 AI 的内容。
5. 用户回复“继续”后从暂停点继续，不重复已经通过的步骤，不要求用户重新描述上下文。
6. 未通过对应脱敏测试的渠道或 result type 保持关闭，不用另一个环境的验收记录代替当前环境证据。

AI 不得要求用户把 App Secret、access token、OAuth token 或 MCP token 发到聊天中，也不得把它们写入命令参数、项目文件、报告、配置或状态。飞书 `folder_token`、目录 token 和滴答 `project_id` 是非敏感目标标识，可由 AI 从官方工具结果中提取并写入被忽略的 `复盘/.result-distribution-config.json`；除非官方工具无法取得，否则不要求用户手动复制。API 由现有官方 `lark-cli` 或对应区域官方 MCP 提供，不要求用户自行提供 API。

结束时必须分别报告：

- **本地状态**：是否能真实写入并重新读取报告。
- **飞书状态**：是否完成授权与目录绑定，脱敏文档是否位于正确目录且用户可见，已开启哪些结果类型。
- **滴答状态**：区域、官方 MCP、唯一“知己行动”清单和 `project_id` 是否有效，脱敏任务的标题、截止时间和清单是否正确，已开启哪些结果类型。
- **唯一下一步**：仍有缺口时，只给当前最小的一项用户操作；全部通过时明确说明无需继续设置。
