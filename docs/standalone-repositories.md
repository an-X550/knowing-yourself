...
| ReflectLoop Agent Skill 用户版分发包 | `zhiji-user/` | <https://github.com/an-X550/Reflectloop-Agent-Skill> | `main` | `git -C zhiji-user push origin main` |
...
| ReflectLoop DSH Plugin | `apps/zhiji-dsh-plugin/`（主仓库快照） | <https://github.com/an-X550/Reflectloop-DSH-Plugin> | `main` | 在独立仓库工作目录执行 `git push origin main` |
...
- ReflectLoop 是英文项目名，“知己”是中文产品名，不再作为独立仓库名称的一部分。四个 GitHub 仓库分别使用 `Reflectloop`、`Reflectloop-Agent-Skill`、`Reflectloop-Desktop-Agent` 和 `Reflectloop-DSH-Plugin`；本地快照目录与 DSH 内部包 ID 保持兼容，不要求与远程仓库同名。
...
前一条应返回 `https://github.com/an-X550/Reflectloop.git`，后一条应返回 `https://github.com/an-X550/Reflectloop-Agent-Skill.git`。桌面端独立仓库应返回 `https://github.com/an-X550/Reflectloop-Desktop-Agent.git`；DSH 插件独立仓库应返回 `https://github.com/an-X550/Reflectloop-DSH-Plugin.git`。桌面端和 DSH 插件应在各自独立仓库的工作目录中核对和推送，不要把主仓库里的快照目录当作独立 Git 根目录。
