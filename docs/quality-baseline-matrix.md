...
| weekly-monthly-review | 周/月复盘 | multi-perspective, six-question-report, adaptive-depth, chat-summary, user-response-section, output-path | `tests/review-workflow-contract.tests.ps1` 检查 synthesis 返回全文、摘要提取、周报用户回应区和判断驱动展开；`tests/quality-baseline.tests.ps1` 检查多视角并行、六问契约、用户回应入口与输出路径 key | 用真实素材跑一次 `/weekly-review` 与 `/monthly-review` |
| project-review | 项目复盘 | conversation-materials, project-filename, six-question-report, adaptive-depth, chat-summary, output-path | `tests/quality-baseline.tests.ps1` 检查 `sanitizeProjectSlug`、`output.project_report`、当前对话材料消费提示和项目复盘命令模式；`tests/review-workflow-contract.tests.ps1` 检查项目综合代理的判断驱动展开 | 用真实项目材料跑一次 `/project-review` |
