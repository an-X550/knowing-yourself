...
| weekly-monthly-review | 周/月复盘 | multi-perspective, six-question-report, chat-summary, output-path | `tests/review-workflow-contract.tests.ps1` 检查 synthesis 返回全文和摘要提取；`tests/quality-baseline.tests.ps1` 检查多视角并行、六问契约与输出路径 key | 用真实素材跑一次 `/weekly-review` 与 `/monthly-review` |
