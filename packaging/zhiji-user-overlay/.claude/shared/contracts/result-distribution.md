...
只允许 create-task 写入。禁止绑定或调用 list/get/search/update/complete/delete，以及任何历史、习惯、项目或完成状态查询能力；不根据第三方状态评价用户，也不建立回写、双向同步或自动复盘。完成判断只读取后续日志，不读取滴答状态。本轮只定义语义，不调用或模拟 MCP。
...
系统不自动创建、查询、迁移或修复“知己行动”清单。清单不存在或不可访问时本次创建失败，只给一条设置指引；用户修复后明确重试失败项。
...
调用唯一绑定的 create-task 操作时逐项传入最终标题、截止日期或时间以及目标清单。每项任务分别记录来源类型和本地来源标识、`normalized_title`、`exact_due_date_or_time`、`action_key`、`status`、远端 `task_id`、`attempted_at` 与规范化 `error_code`；不得记录 description、完成记录、修改历史、MCP token、完整响应或无关任务信息。一项失败不阻止剩余合格项，也不改变飞书或本地结果。
