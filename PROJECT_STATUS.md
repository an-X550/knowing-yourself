...
| 日期 | 决策 | 理由 |
|------|------|------|
| 2026-08-22 | 采用 ReflectLoop 作为英文项目名，保留“知己”作为中文产品名 | `knowing-yourself` 是早期直译，不能稳定表达软件身份；单独使用拼音 `Zhiji` 又缺少英文语义。ReflectLoop 直接对应“记录—复盘—行动—验证—校准”的核心闭环，并可扩展为主仓库与 Desktop 等组件名称。AI、Agent、模型和具体运行入口继续作为实现手段，不进入产品根定义；主仓库与桌面独立仓库已分别改名为 `Reflectloop` 和 `ReflectLoop-Desktop`，GitHub 简介、远程地址与包 metadata 已完成同步。 |
