...
| 可选结果分发 | 飞书白名单已配置，待日常新产物观察 | 新生成的正式复盘、人生设计、已确认主题思考及明确收录的收藏/附件可按固定“知己”目录沉淀到飞书；滴答仍只消费报告内已有的合格行动。用户在本次生成请求中说“仅本地”时同时跳过两个渠道，不在报告、配置或状态中留下永久标记。本地文件始终权威，不扫描电脑，不上传日志、画像、中间分析、配置或状态。飞书根目录及固定分类已真实核验，历史 60 项已按人工确认完成同步；仍待三次真实新写入与首次真实非 Markdown 收藏附件观察。日常写入使用 bot，不要求逐次用户授权；只在自动授予权限失败或用户实际不可见时恢复授权。 |
