...
| DeepSeek Harness Agent 升级 | 阶段 0、A、B 已完成；阶段 C 待实施 | DSH `0.1.0-rc.8` 发布包在独立 Electron Utility Process 运行，Main Process 以 MessagePort 代理模型并独占 API Key。Agent 可通过 Main Process Zod 校验的高层工具读取脱敏日志/复盘/项目/主题/验证模式、受控搜索与读源，并展示既有页面的导航和结果卡；不暴露路径、URL、Shell、任意文件或写入捷径。每日反馈、周/月复盘等正式能力仍由既有服务、预览/确认与仓储链路负责；阶段 C 将基于实际使用证据决定哪些工作流适合高层复用。接入面和验证证据见 [`dsh-integration-notes`](apps/zhiji-desktop/docs/dsh-integration-notes.md)。 |
