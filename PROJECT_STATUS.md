...
| Agent 本地历史检索与能力自述 | v2.5.0 已实现词法检索工程入口；用户可见证据待补 | Agent 会说明 DSH 上下文组件、Function Calling、有限多步工具规划和内部结构化校验的真实边界；只读 `zhiji.memory.search` 以字符串包含方式复用日志、复盘和已确认验证模式仓储，返回有限脱敏摘录。它不是 RAG 或完整长期记忆，中文复合查询、同义召回和大规模性能尚未验证，Agent 页目前只显示工具活动标签。下一步只做当前回合证据卡片，不新增记忆副本、向量数据库、外部服务或自动写入。见 [`agent-capabilities-first-principles-analysis`](docs/2026-08-22-agent-capabilities-first-principles-analysis.md)、[`agent-memory-search`](docs/specs/2026-08-22-agent-memory-search.md)、[`agent-evidence-cards`](docs/specs/2026-08-22-agent-evidence-cards.md)。 |
