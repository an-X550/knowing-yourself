...
| 项目 | 状态 | 当前能力或证据缺口 |
|------|------|------|
| 日反馈闭环 | 已完成 | 日志粘贴与 `/daily-review` 统一进入 `daily-analyzer`；正式反馈新写入、复读与验证沉淀成功后，两入口统一进入可选结果分发；输出按单洞察、单行动、可观察预测收敛，输入不足按 A-D 证据等级降级。 |
...
| 用户版分发与边界 | 已完成 | overlay、manifest、导出链路、Codex 日志路由、Windows Stop Hook、年度目录和分发边界测试已具备；README 现按“功能地图 → 本地首个闭环 → AI 引导外部分发 → 必要人工授权 → 日常指令”组织，并明确主项目验收不能替代逐用户环境验收。分发配置和状态 JSON 已加入用户版 Git 忽略；仍需真实用户干运行。 |
