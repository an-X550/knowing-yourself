...
| Windows 桌面客户端 | 任务型入口、日志完整性与必要数据生命周期已完成；目标用户理解测试和安装矩阵待人工验收 | 一级导航为开始、日志、复盘、项目、设置。日志可选日期、同日多条、显式编辑并从历史生成日反馈；日志和复盘可逐条移入 Windows 回收站且不级联删除来源。项目名全局唯一，支持重命名、归档、恢复，无关联日志时可移入回收站。AI 连接测试通过后保存并刷新状态，也可清除当前服务商 Key。下拉控件统一视觉并保留原生行为。数据目录、个人背景、可验证备份、安全 IPC 与 OpenAI 兼容接口保持不变。30 个测试文件、103 项测试、类型检查、Lint（0 error）、真实 Electron 主链路和 Windows x64 打包已通过。关闭窗口时的草稿保护、目录迁移、合并恢复、Profile 注入 AI、安装/升级/卸载和 Windows 10 仍未实现或验证。 |
