...
| Windows 桌面客户端 | 任务型入口与 P0 本地数据闭环已完成；目标用户理解测试和安装矩阵待人工验收 | 一级导航为开始、日志、复盘、项目、设置；开始页按本地记录确定性推荐一个下一步，并提供三个低权重入口。日志/复盘历史归入各自页面，项目复盘携带项目上下文；未配置 AI 时主操作只保存日志。数据目录、个人背景、可验证备份、安全 IPC 与 OpenAI 兼容接口保持不变。28 个测试文件、71 项测试、类型检查、Lint、真实 Electron 主链路和 Windows x64 打包已通过。目录迁移、合并恢复、Profile 注入 AI、安装/升级/卸载和 Windows 10 仍未实现或验证。 |
