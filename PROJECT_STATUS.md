...
| DeepSeek Harness Agent 升级 | 阶段 0、阶段 A 已完成；阶段 B 待实施 | DSH `0.1.0-rc.8` 发布包在独立 Electron Utility Process 运行最小 Agent loop，Main Process 以 MessagePort 代理模型流并独占 API Key；Agent 页提供会话、流式消息、停止与中文崩溃降级。阶段 A 没有迁移或改写每日反馈、周/月复盘等领域能力；阶段 B 将只读、受 Zod 校验地复用现有服务。接入面和验证证据见 [`dsh-integration-notes`](apps/zhiji-desktop/docs/dsh-integration-notes.md)。 |
