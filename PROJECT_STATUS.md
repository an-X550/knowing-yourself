...
| 项目 | 状态 | 当前能力或证据缺口 |
|------|------|------|
| Agent 打包启动兼容性 | 已修复 | Forge 生产 asar 已保留外置 DSH 依赖 `zod`、`@standard-schema/spec`；打包后 Agent 新建会话 E2E 通过。Utility 启动错误现在保留有限、脱敏诊断，不再只显示“运行已停止”。当前版本 `2.5.0` 的最终人工验收应使用本次重新生成的安装包，不使用依赖清单不同的旧安装目录。 |
...
| Agent 本地历史检索与能力自述 | v2.5.0 已实现字符串检索入口；中文召回与用户可见证据待补 | Agent 会说明 DSH 上下文组件、Function Calling、有限多步工具规划和内部结构化校验的真实边界；只读 `zhiji.memory.search` 当前以字符串包含方式复用日志、复盘和已确认验证模式仓储，返回有限脱敏摘录。它不是 RAG 或完整长期记忆；自然中文长问题会被当成整段词项，现有测试只覆盖单关键词，Agent 页也只显示活动标签。已确认按“MiniSearch 内存 BM25+ + `Intl.Segmenter`/CJK 词片 + 受限候选查询 → 当前回合证据卡片”实施；不新增持久化索引、向量数据库、外部服务或自动写入。见 [`agent-capabilities-first-principles-analysis`](docs/2026-08-22-agent-capabilities-first-principles-analysis.md)、[`agent-memory-search`](docs/specs/2026-08-22-agent-memory-search.md)、[`agent-evidence-cards`](docs/specs/2026-08-22-agent-evidence-cards.md)。 |
...
- [ ] Agent 中文历史检索与证据卡片（需求已确认、待实现）：先以 MiniSearch 内存 BM25+、`Intl.Segmenter`、CJK 二元词片和至多 3 个受限候选查询修复自然中文复合/有限同义召回，再把 Main Process 已校验命中结果以当前回合只读卡片展示，默认 3 条、最多 8 条。索引不持久化，不新增向量 RAG、MCP、多模态或权限；使用脱敏夹具验收事实、复合/同义、模式与冲突四场景（见 [`agent-evidence-cards`](docs/specs/2026-08-22-agent-evidence-cards.md)、[`execution-plan`](docs/2026-08-22-agent-evidence-cards-execution-plan.md) 与 [`execution-prompt`](docs/2026-08-22-agent-retrieval-evidence-execution-prompt.md)）。
