...
zhiji（中文名“知己”）是一个面向中文个人日志的本地优先 AI 复盘系统。它帮助你从真实记录中整理证据、发现模式、形成一个低成本行动，并在后续记录中验证和校准认识。
...
zhiji 是项目总称；Windows 客户端、Codex/Claude、DSH 插件和 WorkBuddy 是不同运行入口，不会自动共享同一份数据。

| 你的使用方式 | 推荐入口 | 说明 |
| --- | --- | --- |
| 希望直接安装 Windows 应用 | [zhiji Desktop Agent（知己 Windows 客户端）](https://github.com/an-X550/zhiji-Desktop-Agent) | 提供受控 Agent、开始、日志、复盘、项目和设置六个可视化入口；数据默认保存在本机；主仓库快照见 [桌面端说明](apps/zhiji-desktop/README.md) |
| 使用 Codex 或 Claude 读取本地 Markdown | [zhiji Agent Skill（知己用户版）](https://github.com/an-X550/zhiji-Agent-Skill) | 下载或复制完整工作区后，Codex 可直接使用自然语言；Claude 保留 Slash Command 兼容入口；本地快照见 [用户版使用指南](zhiji-user/README.md) |
| 已安装 DeepSeek Harness | [zhiji DSH Plugin（知己 DSH 插件）](https://github.com/an-X550/zhiji-DSH-Plugin) | 在 DSH 会话中处理粘贴材料，或在显式配置后按日期范围只读聚合日志；主仓库快照见 [插件说明](apps/zhiji-dsh-plugin/README.md) |
...
写日志时，人容易记住当天的感受，却很难看见跨时间重复出现的模式；即使得到一个解释，也常常没有后续验证。zhiji 将不同周期的复盘收敛到同一条闭环：
...
AI 和 Agent 是完成复盘闭环的手段，不是 zhiji 的产品目的。不同入口的实现不同，但都应遵守相同边界：
...
如果你想从拖延、情绪反复、精力波动、项目复盘或长期方向冲突中找出可检验的规律，zhiji 适合你。一段真实、简短的记录就足够开始，不需要先养成完美的日记习惯。
...
本仓库是 zhiji 的主仓库，保存 Skill、共享契约、运行定义、用户分发包和独立组件的迁移快照；`.claude/` 是 Skill/CLI 侧的唯一运行真相。
...
- 仓库改名经过、旧名称兼容与排障：[docs/2026-08-22-zhiji-repository-naming-migration.md](docs/2026-08-22-zhiji-repository-naming-migration.md)
