...
[![Version](https://img.shields.io/badge/版本-v1.6.11-green)](VERSION)
...
| 主题思考 | 想深入讨论任意长期问题，并在以后继续沿用或修正认识 | 当前行动卡、当前问题、当前判断、依据来源与思考校正、值得保留的行动、下一次怎么做、我的观点演化路径 |
...
直接用自然语言开始探讨。形成相对明确的认识后，AI 会先用“当前行动卡 + 六个陈述式板块”归纳：当前问题、当前判断、依据来源与思考校正、值得保留的行动、下一次怎么做、我的观点演化路径。它必须区分事实、推断与建议，并为严肃挑战给出反例、证据缺口或替代解释。只有你确认后，才会在 `关于我/思考/` 动态创建或更新对应主题文件。这里没有固定主题清单。

主题板块是帮助思考闭环的结构清单，不是等长作文。系统会按证据、问题复杂度和行动需要决定哪一节展开：执行信息前置，分析信息归位；有明确行动、高风险判断或反例时写细；没有新证据时短写并说明原因，不为了填满标题增加泛泛内容。
