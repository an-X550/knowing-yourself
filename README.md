...
[![Version](https://img.shields.io/badge/版本-v1.9.5-green)](VERSION)
...
| 主题思考 | 想深入讨论任意长期问题，并在以后继续沿用或修正认识 | 首次讨论先形成“问题 → 事实/约束 → 判断 → 下一步”；确认沉淀后，按用途选择 0–6 或短结构 |
...
主题板块是帮助思考闭环的结构清单，不是等长作文。默认结构按证据、问题复杂度和行动需要决定哪一节展开；自定义结构也不能绕过证据、行动与验证。执行信息前置，分析信息归位；行动卡只保留当前最必要的行动及观察或停止条件，按需增减，不默认写成一周反馈；“值得保留的行动”统一写成可长期重复使用的行动表；没有新证据时短写并说明原因，不为了填满标题增加泛泛内容。
