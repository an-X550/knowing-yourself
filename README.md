...
[![Version](https://img.shields.io/badge/版本-v1.9.3-green)](VERSION)
...
干预实验：明天只写出第一步，并完成一个不超过 5 分钟的动作。
实验结果：仍然没有启动。
...
| 收藏吃灰库收录 | 想保存一篇以后可能有用的文章或摘录 | 固定格式的摘要、关键词、原文与链接 |
| `/review` | 不确定该做哪种复盘 | 根据材料与时间范围路由到合适入口 |
| `/daily-review` | 想完成当天最短闭环 | 单日反馈、昨日行动验证、次日实验 |
| `/journal-coach` | 想改善连续几天的记录质量 | 分析就绪度、六步法习惯、优先改进项 |
| Codex 自然语言周报（`/weekly-review` 兼容） | 想检查一周趋势与行动结果 | 基于复盘六问的周度综合复盘，写入 `output.weekly_report` |
| Codex 自然语言月报（`/monthly-review` 兼容） | 想识别更稳定的主题、节奏和反例 | 月度证据包、主题归并、验证沉淀复盘，写入 `output.monthly_report` |
| Codex 自然语言项目复盘（`/project-review` 兼容） | 完成一个项目、版本或阶段后 | 目标、结果、正负原因、重来演练与后续规划，写入 `output.project_report` |
| `/yearly-review` | 已积累足够月报，希望回看全年 | 年度轨迹、关键转折、持续挑战与下一年方向 |
| `/life-design` | 长期方向、目标或生活结构出现冲突 | 方向校准、能量地图、人生原型与 7 天实验 |
| 主题思考 | 想深入讨论任意长期问题，并在以后继续沿用或修正认识 | 默认 0-6 编号阅读路径；只有用户确认且复核证明能减少重复时，才采用保留闭环语义的自定义结构 |
...
想收录有用文章时，直接说“收录到收藏吃灰库/健身：”并附上文章、摘录或可读取链接。AI 会保存到 `关于我/收藏吃灰库/`，统一写成标题、摘要、关键词、原文/摘录全文和原文链接五部分。

### 主题思考：让观点持续演化

适合：你想与 AI 深入讨论信息输入、工作与健康、关系边界或任何其他长期问题，并希望以后继续沿用这次形成的认识。

直接用自然语言开始探讨。首次讨论默认先收敛为“问题 → 事实/约束 → 判断 → 下一步”，不预填 0–6；只有你确认沉淀后，持续行动、验证或观点演化类主题才使用带动态结论标题的 0–6 阅读路径。地图、准则、对照和边界类主题可用更短结构，但仍保留范围转向、证据校正与验证或等待条件。它必须区分事实、推断与建议，并为严肃挑战给出反例、证据缺口或替代解释。确认后的新认知会先按对当前判断的影响分类，再重组受影响论证以避免机械追加；篇幅按理解所需决定。只有你确认后，才会在 `关于我/思考/` 动态创建或更新对应主题文件。这里没有固定主题清单。

主题板块是帮助思考闭环的结构清单，不是等长作文。默认结构按证据、问题复杂度和行动需要决定哪一节展开；自定义结构也不能绕过证据、行动与验证。执行信息前置，分析信息归位；行动卡只保留短期动作、长期锚点、触发场景和停止线，不默认写成一周反馈；“值得保留的行动”统一写成可长期重复使用的行动表；没有新证据时短写并说明原因，不为了填满标题增加泛泛内容。
...
| 回顾本周 | `生成 2026-W28 周报` | `/weekly-review`（Claude 兼容入口） |
| 回顾本月 | `生成 2026 年 6 月月报` | `/monthly-review`（Claude 兼容入口） |
| 回顾项目 | `对 X 做项目复盘` | `/project-review`（Claude 兼容入口） |
| 回顾全年 | 帮我生成年度成长回顾 | `/yearly-review` |
| 校准长期方向 | 我想重新检查最近的生活方向 | `/life-design --quick` |

## 系统如何工作

Codex 可直接使用上表的自然语言入口；Claude slash command 仅为兼容入口，不是 Codex 的前置条件。周、月、项目报告仍按 `.claude/shared/paths.md` 的 `output.*_report` 写入，并在证据不足时明确降级为部分复盘，不伪造视角或证据。
