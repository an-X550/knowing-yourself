...
[![Version](https://img.shields.io/badge/版本-v1.8.4-green)](VERSION)
...
直接用自然语言开始探讨。形成相对明确的认识后，AI 默认会先用“0-6 编号阅读路径”归纳：0. 当前行动卡、1. 当前问题、2. 当前判断、3. 依据来源与思考校正、4. 值得保留的行动、5. 行动验证与复查、6. 我的观点演化路径。只有在第一性原理复核表明标准结构会重复现有主题或遮蔽调用入口，且你明确确认后，才会采用仍保留前置入口、范围转向、证据校正与验证条件的自定义结构。它必须区分事实、推断与建议，并为严肃挑战给出反例、证据缺口或替代解释。只有你确认后，才会在 `关于我/思考/` 动态创建或更新对应主题文件。这里没有固定主题清单。
