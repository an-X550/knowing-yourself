...
[![Version](https://img.shields.io/badge/版本-v1.6.21-green)](VERSION)
...
| 主题思考 | 想深入讨论任意长期问题，并在以后继续沿用或修正认识 | 0-6 编号阅读路径：当前行动卡、当前问题、当前判断、依据来源与思考校正、值得保留的行动、下一次怎么做、我的观点演化路径 |
...
直接用自然语言开始探讨。形成相对明确的认识后，AI 会先用“0-6 编号阅读路径”归纳：0. 当前行动卡、1. 当前问题、2. 当前判断、3. 依据来源与思考校正、4. 值得保留的行动、5. 下一次怎么做、6. 我的观点演化路径。它必须区分事实、推断与建议，并为严肃挑战给出反例、证据缺口或替代解释。只有你确认后，才会在 `关于我/思考/` 动态创建或更新对应主题文件。这里没有固定主题清单。

主题板块是帮助思考闭环的结构清单，不是等长作文。系统会按证据、问题复杂度和行动需要决定哪一节展开：执行信息前置，分析信息归位；行动卡只保留短期动作、长期锚点、触发场景和停止线，不默认写成一周反馈；“值得保留的行动”统一写成可长期重复使用的行动表；没有新证据时短写并说明原因，不为了填满标题增加泛泛内容。
