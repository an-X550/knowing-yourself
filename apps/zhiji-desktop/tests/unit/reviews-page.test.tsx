...
beforeEach(() => { window.zhiji = { reviews: { preview: vi.fn(async () => ({ token: 'token', type: 'weekly', start: '2026-08-10', end: '2026-08-16', sources: [{ id: 'journal_a1', date: '2026-08-13', excerpt: '真实材料' }] })), generatePeriodic: vi.fn(async () => ({ kind: 'review', review: { schemaVersion: 1, id: 'review_a1', type: 'weekly', periodStart: '2026-08-10', periodEnd: '2026-08-16', sourceIds: ['journal_a1'], projectId: null, provider: 'openai-compatible', model: 'test', promptVersion: 'periodic-review-v1', createdAt: '2026-08-13T00:00:00.000Z', body: '本周有效行动' } })), previewInsight: vi.fn(async (input) => ({ token: 'insight-token', ...input, sources: [{ id: 'journal_a1', date: '2026-08-13', excerpt: '深度材料' }] })), generateInsight: vi.fn(async (input) => ({ schemaVersion: 1, id: 'review_insight', type: input.type, periodStart: input.start, periodEnd: input.end, sourceIds: ['journal_a1'], projectId: null, provider: 'openai-compatible', model: 'test', promptVersion: `${input.type}-v1`, createdAt: '2026-08-13T00:00:00.000Z', body: '深度洞察' })), delete: vi.fn(async () => undefined), cancel: vi.fn(), list: vi.fn(), generateDaily: vi.fn() } } as unknown as Window['zhiji']; });
