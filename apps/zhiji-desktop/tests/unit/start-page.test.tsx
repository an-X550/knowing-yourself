...
describe('StartPage', () => {
  ...
  it('has no free-text intent router: the page only offers the deterministic next step and existing entrances', () => {
    render(<StartPage journals={[]} reviews={[]} hasApiKey onNavigate={vi.fn()}/>);
    expect(screen.queryByLabelText('意图描述')).not.toBeInTheDocument();
    ...
    expect(document.querySelector('.intent-card')).toBeNull();
  });
});
