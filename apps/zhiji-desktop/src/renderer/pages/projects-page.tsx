...
export function ProjectsPage({ projects, journals, onRefresh, onNavigate }: Props) {
  ...
  return <>
    ...
    {confirmDelete && <div className="archive-confirm"><p>只移除项目，不会删除日志。</p><Button variant="ghost" onClick={() => setConfirmDelete(false)}>取消</Button><Button variant="danger" onClick={() => void remove()}>确认移除</Button></div>}
  </>;
}
