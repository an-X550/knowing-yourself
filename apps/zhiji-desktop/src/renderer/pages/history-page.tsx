...
export function RecordBrowser({ journals, reviews = [], projects = [], allowedKinds, onEditJournal }: { journals: Journal[]; reviews?: Review[]; projects?: Project[]; allowedKinds: HistoryItem['kind'][]; onEditJournal?(id: string): void }) {
  ...
  return <>
    ...
    {filtered.length ? <div className="history-layout"><section className="history-list">{filtered.map((item) => <button key={item.id} className={item.id === selected?.id ? 'is-active' : ''} onClick={() => setSelectedId(item.id)} aria-label={`${item.title} ${labels[item.kind]}`}><time>{item.date}</time><div><strong>{item.title}</strong><span>{labels[item.kind]}</span></div></button>)}</section>{selected && <HistoryReader item={selected} onEdit={onEditJournal}/>}</div> : <EmptyState title="没有符合筛选条件的记录" description="尝试清空搜索词或调整筛选条件。"/>}
  </>;
}
