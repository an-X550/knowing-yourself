...
export function TodayPage({ journals, projects, reviews, intent, hasApiKey = true, onRefresh, onNavigate, onDirtyChange }: { journals: Journal[]; projects: Project[]; reviews: Review[]; intent?: NavigationIntent; hasApiKey?: boolean; onRefresh(): Promise<void> | void; onNavigate(target: NavigationTarget): void; onDirtyChange?(dirty: boolean): void }) {
  ...
  const confirmDiscard = () => !dirty || window.confirm('这条日志还没有保存，确定放弃吗？');
  ...
  const generate = async () => {
    ...
    try {
      const journal = body.trim() ? await save() : null;
      ...
      await onRefresh(); setReviewState('success'); setReviewMessage(result.review.body); setDailyReviewBody(result.review.body);
  ...
  return <>
    ...
    <PageHeader title={section === 'compose' ? '写一条日志' : '过去日志'} description={section === 'compose' ? '选择今天或过去日期，真实地写就够了。' : '按日期找到过去的原始记录。'} action={<div className="page-tabs"><button className={section === 'compose' ? 'is-active' : ''} onClick={() => setSection('compose')}>写日志</button><button className={section === 'records' ? 'is-active' : ''} onClick={() => { if (confirmDiscard()) setSection('records'); }}>过去日志</button></div>}/>
    {section === 'records' ? <>{reviewMessage && <StatusBanner tone={reviewState === 'error' ? 'error' : reviewState === 'success' ? 'success' : 'info'}>{reviewMessage}</StatusBanner>}<RecordBrowser journals={journals} reviews={reviews} projects={projects} allowedKinds={['journal']} onDelete={(item) => { setDeleteId(item.id); setReviewMessage(''); }} onGenerateDaily={(selectedDate) => void generateForDate(selectedDate)} onEditJournal={(id) => { if (!confirmDiscard()) return; const journal = journals.find((item) => item.id === id); if (!journal) return; setEditing(journal); setDate(journal.date); setBody(journal.body); setProjectId(journal.projectIds[0] ?? ''); setSection('compose'); setSaveState('idle'); }}/>{deleteId && <div className="archive-confirm"><p>已有复盘不会同步删除。</p><Button variant="ghost" onClick={() => setDeleteId(null)}>取消</Button><Button variant="danger" onClick={() => void removeJournal()}>确认移除</Button></div>}</> : <>
      <div className="today-grid"><section className="card editor-card"><Field label="日志日期"><input aria-label="日志日期" type="date" max={today} value={date} onChange={(event) => { setDate(event.target.value); setSaveState('idle'); }}/></Field><Field label="关联项目（可选）"><select value={projectId} onChange={(event) => { setProjectId(event.target.value); setSaveState('idle'); }}><option value="">不关联项目</option>{projects.filter((item) => item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="日志内容"><textarea ref={editorRef} aria-label="日志内容" value={body} onChange={(event) => { setBody(event.target.value); setSaveState('idle'); }} placeholder="发生了什么？你做了什么？结果怎样？"/></Field><div className="editor-meta"><span className="tag">{date === today ? '今天' : date}</span><span className="tag">{body.length} 字</span></div>{date < today && <p className="muted">补写历史日志只保存，不会自动生成 AI 反馈。</p>}{saveMessage && <StatusBanner tone={saveState === 'error' ? 'error' : 'success'}>{saveMessage}</StatusBanner>}<div className="button-row">{hasApiKey && date === today && body.trim() && <Button variant="ghost" loading={saveState === 'loading'} onClick={() => void save()}>仅保存日志</Button>}<Button variant="primary" loading={hasApiKey && date === today ? reviewState === 'loading' : saveState === 'loading'} disabled={!body.trim() && !(hasApiKey && date === today && journals.some((item) => item.date === today))} onClick={() => void (hasApiKey && date === today ? generate() : save())}>{hasApiKey && date === today ? (body.trim() ? '保存并生成今日反馈' : '生成今日反馈') : '保存日志'}</Button></div>{reviewMessage && <StatusBanner tone={reviewState === 'error' ? 'error' : reviewState === 'success' ? 'success' : 'info'}>{reviewMessage}</StatusBanner>}{dailyReviewBody && <div className="button-row"><Button variant="ghost" onClick={() => onNavigate({ view: 'topics', intent: { type: 'topics.start', question: deriveTopicQuestion(dailyReviewBody), contextExcerpt: dailyReviewBody } })}>就这个深入探讨</Button></div>}</section>
