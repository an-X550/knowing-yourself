...
export function TodayPage({ journals, projects, reviews, intent, hasApiKey = true, onRefresh, onNavigate, onDirtyChange }: { journals: Journal[]; projects: Project[]; reviews: Review[]; intent?: NavigationIntent; hasApiKey?: boolean; onRefresh(): Promise<void> | void; onNavigate(target: NavigationTarget): void; onDirtyChange?(dirty: boolean): void }) {
  ...
  return <>
    ...
    <PageHeader title={section === 'compose' ? '写一条日志' : '过去日志'} description={section === 'compose' ? '选择今天或过去日期，真实地写就够了。' : '按日期找到过去的原始记录。'} action={<div className="page-tabs"><button className={section === 'compose' ? 'is-active' : ''} onClick={() => setSection('compose')}>写日志</button><button className={section === 'records' ? 'is-active' : ''} onClick={() => setSection('records')}>过去日志</button></div>}/>
