...
export function TodayPage({ journals, projects, reviews, intent, hasApiKey = true, onRefresh, onNavigate }: { journals: Journal[]; projects: Project[]; reviews: Review[]; intent?: NavigationIntent; hasApiKey?: boolean; onRefresh(): Promise<void> | void; onNavigate(target: NavigationTarget): void }) {
  ...
  if (!hasApiKey) return <><div className="ai-hint"><span>先保存日志也可以</span><Button variant="secondary" onClick={() => onNavigate({ view: 'settings' })}>配置 AI</Button></div><TodayPage journals={journals} projects={projects} reviews={reviews} intent={intent} hasApiKey onRefresh={onRefresh} onNavigate={onNavigate}/></>;
