...
import { PatternPanel } from '../features/patterns/pattern-panel';
...
export function ReviewsPage({ projects, reviews = [], intent, onRefresh = () => undefined }: { projects: Project[]; reviews?: Review[]; intent?: NavigationIntent; onRefresh?(): Promise<void> | void }) {
  ...
  const generate = async () => { if (!input || !preview) return; setState('loading'); setMessage(''); try { if (isInsight) { const next = await window.zhiji.reviews.generateInsight({ ...input, previewToken: preview.token } as Parameters<typeof window.zhiji.reviews.generateInsight>[0]); setResult(next); } else { const next = await window.zhiji.reviews.generatePeriodic({ ...input, previewToken: preview.token } as Parameters<typeof window.zhiji.reviews.generatePeriodic>[0]); if (next.kind === 'review') { setResult(next.review); } else { setResult(null); setState('error'); setMessage(next.question); return; } } setState('success'); setMessage('复盘已保存到本机'); } catch (reason) { setState('error'); setMessage(`生成失败：${reason instanceof Error ? reason.message : '请检查设置'}`); } };
