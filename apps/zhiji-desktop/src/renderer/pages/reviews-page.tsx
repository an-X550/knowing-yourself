...
export function ReviewsPage({ projects, reviews = [], intent, onRefresh = () => undefined }: { projects: Project[]; reviews?: Review[]; intent?: NavigationIntent; onRefresh?(): Promise<void> | void }) {
  ...
  return <>
    ...
    {section === 'history' ? <>{message && <StatusBanner tone={state === 'error' ? 'error' : state === 'success' ? 'success' : 'info'}>{message}</StatusBanner>}<RecordBrowser journals={[]} reviews={result && !reviews.some((item) => item.id === result.id) ? [result, ...reviews] : reviews} projects={projects} allowedKinds={['daily', 'weekly', 'monthly', 'project']} onDelete={(item) => { setDeleteId(item.id); setMessage(''); }}/>{deleteId && <div className="archive-confirm"><p>来源日志不会被删除。</p><Button variant="ghost" onClick={() => setDeleteId(null)}>取消</Button><Button variant="danger" onClick={() => void removeReview()}>确认移除</Button></div>}</> : <>
