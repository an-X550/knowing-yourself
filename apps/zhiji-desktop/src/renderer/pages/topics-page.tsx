...
export function TopicsPage() {
  ...
  const start = () => run(async () => {
    const next = await window.zhiji.topics.start({ question: question.trim() });
  ...
  return <div className="page-content topics-page">
    ...
    {discussion && <section className="card topic-discussion">
      ...
      {proposal && <div className="topic-proposal">
        ...
        {proposal.mode === 'update' && <details><summary>差异：既有正文（确认后将被新归纳覆盖）</summary><pre>{proposal.existingBody}</pre></details>}
        <pre>{proposal.summary.body}</pre>
    ...
    <section className="card">
      ...
      {entries.length > 0 && <ul className="topic-index">
        {entries.map((entry) => <li key={entry.topic}>
          ...
          {openTopic?.topic === entry.topic && <pre>{openTopic.body}</pre>}
        </li>)}
      </ul>}
    </section>
