...
export function App() {
  const [view, setView] = useState<'today' | 'reviews' | 'projects' | 'history' | 'settings'>('today');
  ...
  return <div className="app-shell">
    <aside><h1>知己</h1><button onClick={() => setView('today')}>今天</button><button onClick={() => setView('reviews')}>复盘</button><button onClick={() => setView('projects')}>项目</button><button onClick={() => setView('history')}>历史</button><button onClick={() => setView('settings')}>设置</button><small>数据保存在你的电脑上</small></aside>
    <main>{view === 'today' ? <>
