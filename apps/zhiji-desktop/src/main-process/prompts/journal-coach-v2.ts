...
export const JOURNAL_COACH_PROMPT_VERSION = 'journal-coach-v2';
...
2. 六步法写作习惯：事实回顾、重点筛选、感受、思考、行为改进、分享讨论（可选增强）。自由叙事中的等价证据有效，不因缺少固定栏目扣分；未分享只能写“未观察到”。

汇总最稳定证据、最常缺少证据、重复写作问题，只给一个不要求改用固定模板的低摩擦动作。不得做心理诊断或长篇生活建议。只有材料同时出现至少两类方向性信号时才填写 directionWarning，否则返回 null。
