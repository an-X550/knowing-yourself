...
export function validateBusinessArchive(files: Map<string, Buffer>): void {
  ...
  for (const [filePath, content] of files) {
    try {
      if (filePath.startsWith('journals/')) { const parsed = matter(content.toString('utf8')); const item = JournalSchema.parse({ schemaVersion: parsed.data.schema_version, id: parsed.data.id, date: parsed.data.date, createdAt: parsed.data.created_at, updatedAt: parsed.data.updated_at, projectIds: parsed.data.project_ids ?? [], body: parsed.content.trim() }); journals.add(item.id); journalProjects.push({ ids: item.projectIds, path: filePath }); }
