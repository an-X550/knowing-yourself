...
if (Get-Command ConvertTo-ZhijiEntryDecision -ErrorAction SilentlyContinue) {
  ...
  try {
    ...
    if (Get-Command ConvertTo-ZhijiFollowUpDecision -ErrorAction SilentlyContinue) {
      ...
      $followUp.content = "追问：8月10日 那条反馈哪里有证据？"
      ...
      $followUp.content = "展开讲讲"
      Assert-Equal (ConvertTo-ZhijiFollowUpDecision -Event $followUp -AllowedOpenId "ou_owner" -RepoRoot $followUpRoot).action "usage" "bare natural-language follow-up must not rely on implicit routing"
      $followUp.content = "为什么？"
      Assert-Equal (ConvertTo-ZhijiFollowUpDecision -Event $followUp -AllowedOpenId "ou_owner" -RepoRoot $followUpRoot).action "usage" "bare why question must not rely on implicit routing"
      $followUp.content = "追问：展开讲讲"
      Assert-Equal (ConvertTo-ZhijiFollowUpDecision -Event $followUp -AllowedOpenId "ou_owner" -RepoRoot $followUpRoot).action "follow_up" "explicit follow-up with expansion request must route"
  ...
  if (Get-Command Invoke-ZhijiFollowUpDecision -ErrorAction SilentlyContinue) {
    ...
    try {
      ...
      Assert-True ($script:replyText -match '追问：你的问题') "reply must advertise the explicit follow-up format"
      Assert-Equal (Get-ZhijiFollowUpHelpText) '💬 继续追问：发送“追问：你的问题”；例如“追问：展开讲讲”。' "follow-up help must remain a single explicit-format line"
